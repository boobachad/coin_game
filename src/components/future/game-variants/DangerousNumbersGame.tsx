...
export default function DangerousNumbersGame() {
    ...
    const [dangerousNumber, setDangerousNumber] = useState(5)
    const [allowedMoves, setAllowedMoves] = useState([1, 2, 3, 4])
    const [isConfiguring, setIsConfiguring] = useState(true)
    const [showHelper, setShowHelper] = useState(true)
    const [showStats, setShowStats] = useState(true)

    const isDangerousNumber = (num: number) => num % dangerousNumber === 0
    ...
    const handleMakeMove = (coinsTaken: number) => {
        ...
        const isDangerous = isDangerousNumber(newTotalCoins)

        const newGameState: GameState = {
            totalCoins: newTotalCoins,
            currentPlayer: gameState.currentPlayer === 1 ? 2 : 1,
            gameOver: isDangerous,
            winner: isDangerous ? (gameState.currentPlayer === 1 ? 2 : 1) : null,
            moveHistory: [
                    ...
                    <div>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                min="2"
                                max="10"
                                value={dangerousNumber}
                                onChange={(e) => setDangerousNumber(Math.max(2, Math.min(10, Number(e.target.value))))}
                                className="input"
                            />
                            <p className="text-sm text-gray-600 mt-1">
                                Player who makes the remaining coins a multiple of this number loses
                            </p>
                        </div>
                        <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            {allowedMoves.map((move) => (
                                <button
                                    ...
                                    className={`btn ${isDangerousNumber(gameState.totalCoins - move)
                                            ? "btn-danger"
                                            : "btn-primary"
                                        }`}
                                >
                                            ...
                                            {gameState.moveHistory.map((move, index) => (
                                                <tr key={index} className="border-b last:border-b-0">
                                                    ...
                                                    <td className="text-right py-2">
                                                        {move.remainingCoins}
                                                        {isDangerousNumber(move.remainingCoins) && (
                                                            <span className="text-red-500 ml-2">(Dangerous!)</span>
                                                        )}
                                                    </td>
                                                </tr>
                                            ))}
